function create_corrective_model(ref; budget::Int = 5, 
    ...
    @first_stage sp = begin 
        ...
        """ not required for the corrective model - left it for future additions """
...
function save_corrective_control_model_results(ref, cli_args, corrective_model, off_branches, file::AbstractString)
    ...
    total_load_shed_solution = Dict{String,Any}(
        string(i) => round(total_load - round(value.(model[2, :load_scenario], i) |> vec |> sum; digits=4); digits=4) for i in 1:num_scenarios
    )
    ...
    results = Dict{String,Any}(
        ...
        "total_ramping" => total_scenario_ramping_solution,
        "load_shed" => load_shed_solution, 
        "total_load_shed" => total_load_shed_solution
    )
    open(file, "w") do f
